...
public interface ShelfRepository extends CrudRepository<Shelf, String>, JpaRepository<Shelf, String> {
	...
	@Query
	(value = "SELECT * FROM shelf where floor = ?1 and shelfId not in " + 
			...
			" Union (select distinct shelfId from bookingtemp))",
	nativeQuery =true		
	)
	...
	(value = "select * from " + 
			...
			"AND " + 
			"(SELECT DATE_ADD(CURDATE(), INTERVAL + 4 - WEEKDAY(CURDATE()) DAY))) "+ 
	...
	@Query(
	value = "select s.*, sum((weekday(b.enddate)-weekday(b.startDate)+1)) as sum  from " + 
			"((select * from booking where StartDate BETWEEN (SELECT DATE_ADD(CURDATE(), INTERVAL - WEEKDAY(CURDATE()) DAY)) " + 
			"AND (SELECT DATE_ADD(CURDATE(), INTERVAL + 4 - WEEKDAY(CURDATE()) DAY))) Union(select * from bookingtemp)) as b " + 
			"left join shelf as s on b.shelfId=s.shelfId where floor=?1 " + 
			"group by b.shelfId " + 
			"HAVING sum>=5;",
	nativeQuery =true
	)
